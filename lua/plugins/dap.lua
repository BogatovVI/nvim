return {
  {
    ...
    config = function()
      ...
      dap.configurations.rust = {
        {
          ...
          request = "launch",
          program = function()
            return vim.fn.input("Путь к бинарнику: ", vim.fn.getcwd() .. "/target/debug/", "file")
          end,
          cwd = "${workspaceFolder}",
          stopOnEntry = false,
        },
      ...
      vim.fn.sign_define("DapBreakpoint", { text = "\u{f192}", texthl = "DiagnosticError", linehl = "", numhl = "" })
      vim.fn.sign_define("DapStopped", { text = "\u{f061}", texthl = "DiagnosticWarn", linehl = "Visual", numhl = "DiagnosticWarn" })

      -- Поставить/снять точку останова (Breakpoint) на строке
      vim.keymap.set("n", "<leader>db", dap.toggle_breakpoint, { desc = "Поставить точку останова" })
      
      -- Запустить отладку / Продолжить до следующей точки (Continue)
      vim.keymap.set("n", "<leader>dc", dap.continue, { desc = "Запуск / Продолжить отладку" })
